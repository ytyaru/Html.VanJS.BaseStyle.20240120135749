window.addEventListener('DOMContentLoaded', (event) => {
    ...
    window.win.center.children = [
        button({onclick:()=>window.win.right.children[1] = '既存配列要素の値を変更する'}, 'change'),
        button({onclick:()=>window.win.right.children.push(p('既存配列に要素を追加する'))}, 'push'),
        button({onclick:()=>window.win.right.children = [h1('新配列を代入する'), p('配列まるごと更新！')]}, 'new Array'),
        button({onclick:()=>window.win.right.toggleWritingMode()}, '書字方向変更'),
    ]
